// edge-test.mjs — End-to-end CSP test for strict-csp-next on Next.js 15 with the
// EDGE-runtime middleware (strict-csp-next/proxy-edge). Run AFTER
// next build && node postbuild.mjs && next start -p 4400. Usage: node edge-test.mjs
//
// This proves the Edge entry produces the same strict policy the Node proxy does:
// no 'unsafe-inline', no 'unsafe-eval', build-time hashes on the static route, and
// a UNIQUE per-request nonce on the dynamic route. A predictable/static nonce is a
...
// Fetch a route and return its CSP header, cache-control, and analyzed script-src.
async function fetchRoute(path) {
  const res = await fetch(`${BASE}${path}`)
  const html = await res.text()
  const csp = res.headers.get('content-security-policy') || ''
  const cacheControl = res.headers.get('cache-control') || ''
  return { html, csp, cacheControl, scriptSrc: analyzeScriptSrc(scriptSrc(csp)) }
}
...
async function testRoute(browser, path, waitMs = 1500) {
  ...
  // Raw header analysis. For the dynamic route, fetch TWICE so we can prove the
  // nonce is per-request unique; a reused nonce defeats the policy.
...
for (const route of routes) {
  ...
  // Universal assertions — true for EVERY route regardless of render mode.
  const failures = []
  ...
  if (route.kind === 'static') {
    // Static route: covered by build-time hashes from the imported manifest, no
    // nonce, still cacheable.
...
// Summary table.
console.log('\n=== CSP RESULTS (Next 15, Edge middleware) ===\n')
const labels = {
  '/': 'static (hashes from imported manifest; Edge runtime)',
  '/dynamic': 'dynamic (force-dynamic, per-request nonce; Edge runtime)',
}
